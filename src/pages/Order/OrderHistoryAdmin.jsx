...
const OrderHistoryAdmin = () => {
  ...
  const fetchAllOrders = async () => {
    try {
      const response = await fetch('https://capstone2-yk7b.onrender.com/b6/orders/all-orders', {
        method: 'GET',
        headers: {
  ...
  const fetchProductDetails = async (orders) => {
    ...
    try {
      ...
      for (const productId of uniqueProductIds) {
        const response = await fetch(`https://capstone2-yk7b.onrender.com/b6/products/${productId}`, {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json',
            Authorization: `Bearer ${localStorage.getItem('token')}`
